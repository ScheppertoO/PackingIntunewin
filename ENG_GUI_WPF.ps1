...
function Test-IntuneWinAppUtilGUI {
    ...
    try {
        ...
        # Method 1: GitHub Releases API (for ZIP files)
        ...
        # Method 2: Direct URLs (Fallback)
        if (-not $Downloaded) {
            Write-Log "No ZIP found in release, trying direct download URLs..." "Orange"
            ...
            foreach ($Url in $DirectUrls) {
                try {
                    ...
                    if (Test-Path $IntuneTool) {
                        Write-Log "Download successful!" "Green"
                        $Downloaded = $true
                        break
                    }
                } catch {
        ...
        if ($Downloaded -and (Test-Path $IntuneTool)) {
            ...
            Write-Log "All download methods failed!" "Red"
            [System.Windows.Forms.MessageBox]::Show(
                "Download failed! Please manually download IntuneWinAppUtil.exe and place it in the tools folder.",
                "Download Error",
