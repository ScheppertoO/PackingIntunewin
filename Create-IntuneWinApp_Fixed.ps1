...
function Test-ExeUninstallParameters {
    ...
    Write-Host "🧪 Analysiere EXE für Deinstallationsparameter..." -ForegroundColor Yellow
    
    # Sichere Analyse ohne Ausführung problematischer Parameter
    $DetectedParams = @()
    ...
    foreach ($HelpParam in $HelpParams) {
        try {
            ...
            # Help-Parameter nicht unterstützt
        }
    }
    ...
    # 2. EXE-Metadaten prüfen (ohne Ausführung)
    Write-Host "  📋 Prüfe EXE-Metadaten..." -ForegroundColor Gray
    try {
        ...
        Write-Host "  ⚠️ Metadaten-Analyse nicht möglich" -ForegroundColor Gray
    }
    ...
    Write-Host "  ⚠️ Keine spezifischen Parameter erkannt, verwende Standard-Heuristik" -ForegroundColor Yellow
    ...
    Write-Host "  💡 Tipp: Manuelle Überprüfung der Deinstallation nach Bereitstellung" -ForegroundColor Gray
